...
class WideBasic(enn.EquivariantModule):
    ...
    def __init__(self,
        ...
        if rotations in [0, 2, 4]:
            conv = partial(conv3x3, conv_func)
        else:
            conv = partial(conv5x5, conv_func)
        ...
        if stride != 1 or self.in_type != self.out_type:
            self.shortcut = conv1x1(conv_func, self.in_type, self.out_type, stride=stride, bias=False)
...
class Wide_ResNet(enn.EquivariantModule):
    def __init__(self, depth, widen_factor, dropout_rate, num_classes=100,
        ...
        self.conv1 = conv5x5(self._conv_func, r1, r2)
        ...
        self.out_type = self.layer3.out_type
    ...
    def _wide_layer(self, block, planes: int, num_blocks: int, dropout_rate: float, stride: int,
        ...
        for b, stride in enumerate(strides):
            if b == num_blocks - 1:
                ...
                out_f = main_type
            layers.append(block(self._in_type, inner_type, dropout_rate, stride, out_type=out_f, conv_func=self._conv_func))
            self._in_type = out_f
    ...
    def evaluate_output_shape(self, input_shape: Tuple[int, int, int, int]) -> Tuple[int, int, int, int]:
        ...
        return self.layers3(input_shape)
